...
def load_features_df(sar_paths, svc_file):
    ...
    for file_path in tqdm(sar_paths):
        if not file_path.endswith('.tif'): continue
        
        with tif.TiffFile(file_path) as tif_img:
...
def load_labels_df(bouy_survey_fn, swh_model_fn, wspd_model_fn, sar_bouy_df):
    "This function loads the labels for files indexed with sar_name and bouy_name, those pairs are given in teh sar_bouy_df argument"

    with open(bouy_survey_fn,'rb') as f_r:
    ...
    #Filer the sar_bouy_df (index) dataframe to only include the relevant columns
